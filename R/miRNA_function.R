  ...
  plot_rna_struct <- function(x){
    ...
    suppressWarnings(
      if(!file.exists("miRNA_dicerz.txt")){
        write.table(overhang_output, file = paste0(dir, "miRNA_dicerz.txt"), sep = "\t", quote = FALSE, append = T, col.names = T, na = "NA", row.names = F)
      } else {
        write.table(overhang_output, file = paste0(dir, "miRNA_dicerz.txt"), quote = FALSE, sep = "\t", col.names = F, append = TRUE, na = "NA", row.names = F)
      }
    )
