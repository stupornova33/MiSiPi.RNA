...
dicer_overlaps <- function(dicer_dt, helix_df, chrom_name, reg_start){
   ...
   dicer_dt <- dicer_dt %>%
     dplyr::select(-c(count)) %>% data.frame()
   ...
   if(nrow(filter_helix) > 0){
      ...
      #get reads that start or end at a paired position
       i_dat <- i_dat %>%
         dplyr::select(c(paired_pos, width, rname)) %>%
         dplyr::mutate(end = paired_pos + width - 1, rname = chrom_name) %>%
         stats::na.omit(i_dat)
      ...
      j_dat <- j_dat %>%
         dplyr::select(c(paired_pos, width, rname))  %>%
         dplyr::mutate(end = paired_pos + width - 1, rname = chrom_name) %>%
         stats::na.omit(j_dat) %>% dplyr::select(-c(start))
