#' run in process miRNA if four loops
...
four_loops <- function(vienna_vec, orig_loop_pos, converted){
   ...
   tmp_pos1 <- c(tmp_pos[1], tmp_pos[4])
   tmp_pos2 <- c(tmp_pos[2], tmp_pos[5])
   tmp_pos3 <- c(tmp_pos[3], tmp_pos[6])
   
   loop_df <- data.frame(start = c(orig_loop_pos[1],orig_loop_pos[2],orig_loop_pos[3],orig_loop_pos[4]), 
                         stop = c(orig_loop_pos[5], orig_loop_pos[6],orig_loop_pos[7], orig_loop_pos[8]))
   
   #loop1 will be to the left of )..(
   loop1_diff <- tmp_pos1[1] - loop_df$stop[1]
   #loop2 will be between two ferns. I mean, )..(
   loop2_diff <- tmp_pos2[1] - loop_df$stop[2]
   loop3_diff <- tmp_pos3[2] - loop_df$stop[3]
   loop4_diff <- loop_df$start[4] - tmp_pos3[2]
   
   diff_vec <- c(loop1_diff, loop2_diff, loop3_diff, loop4_diff)
   longest <- which(diff_vec == max(diff_vec))
   
   loop_pos <- c(loop_df$start[longest], loop_df$stop[longest])
   split_seq <- expand(converted)
   #get the new vienna and stem seqs
   if(longest == 1){
      left_vienna <- vienna_vec[1:loop_pos[1]]
      right_vienna <- vienna_vec[loop_pos[2]:tmp_pos1[1]]
      left_stem_seq <- split_seq[1:loop_pos[1]]
      right_stem_seq <- split_seq[loop_pos[2]:tmp_pos1[1]]
      l_other_seq <- vector()
      r_other_seq <- split_seq[(tmp_pos[1] + 1):length(split_seq)]
   } else if(longest == 2){
      left_vienna <- vienna_vec[tmp_pos1[1]:loop_pos[1]]
      right_vienna <- vienna_vec[loop_pos[2]:tmp_pos2[1]]
      left_stem_seq <- split_seq[tmp_pos1[1]:loop_pos[1]]
      right_stem_seq <- split_seq[loop_pos[2]:tmp_pos2[1]]
      l_other_seq <- split_seq[1:(tmp_pos1[1] - 1)]
      r_other_seq <- split_seq[(tmp_pos2[1] + 1):length(split_seq)]
   } else if(longest == 3){
      left_vienna <- vienna_vec[tmp_pos2[2]:loop_pos[1]]
      right_vienna <- vienna_vec[loop_pos[2]:tmp_pos3[1]]
      left_stem_seq <- split_seq[tmp_pos2[2]:loop_pos[1]]
      right_stem_seq <- split_seq[loop_pos[2]:tmp_pos3[1]]
      l_other_seq <- split_seq[1:(tmp_pos2[2] - 1)]
      r_other_seq <- split_seq[tmp_pos3[2]:length(split_seq)]
   } else {
      left_vienna <- vienna_vec[tmp_pos3[2]:loop_pos[1]]
      right_vienna <- vienna_vec[loop_pos[2]:length(vienna_vec)]
      left_stem_seq <- split_seq[tmp_pos3[2]:loop_pos[1]]
      right_stem_seq <- split_seq[loop_pos[2]:length(vienna_vec)]
      l_other_seq <- split_seq[1:tmp_pos3[1]]
      r_other_seq <- vector()
   }
