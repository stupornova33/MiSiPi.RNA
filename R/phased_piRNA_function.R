...
phased_piRNA_function <- function(strand, chrom_name, reg_start, reg_stop, input_file, logfile, dir, plot_output){
  ...
  suppressWarnings(
    if(!file.exists("phased_piRNA_zscores.txt")){
      write.table(phased_output, file = paste0(dir, "phased_piRNA_zscores.txt"), sep = "\t", quote = FALSE, append = FALSE, col.names = F, na = "NA", row.names = F)
    } else {
      write.table(phased_output, file = paste0(dir, "phased_piRNA_zscores.txt"), quote = FALSE, sep = "\t", col.names = F, append = TRUE, na = "NA", row.names = F)
    }
  )
  
  suppressWarnings(
    if(!file.exists("phased26_piRNA_zscores.txt")){
      write.table(phased26_output, file = paste0(dir, "phased26_piRNA_zscores.txt"), sep = "\t", quote = FALSE, append = FALSE, col.names = F, na = "NA", row.names = F)
    } else {
      write.table(phased26_output, file = paste0(dir, "phased26_piRNA_zscores.txt"), quote = FALSE, sep = "\t", col.names = F, append = TRUE, na = "NA", row.names = F)
    }
  )
  ...
  if(plot_output == 'T'){
    ...
    pdf(file = paste0(dir, prefix,"_", strand, "_phased_zscore.pdf"), height = 6, width = 11)
    print(all_plot)
    dev.off()
  }
