...
read_densityBySize <- function(bam_obj, chrom_name, reg_start, reg_stop, input_file, wkdir, logfile) {
   ...
   filter_bamfile <- function(input_file, size1, size2,  strand) {
      ...
      if(strand == "+") {
         filename <- paste0(wkdir, size1, "_", size2, "_pos.bam")
         ...
         filename <- paste0(wkdir, size1, "_", size2, "_neg.bam")
         Rsamtools::filterBam(input_file, destination = filename, filter = filters,
   ...
   del_files <- list.files(path = wkdir, pattern = "\\.bam$")
   del_files <- append(del_files, list.files(path = wkdir, pattern = "\\.bai$"))

   unlink(paste0(wkdir, del_files), force = TRUE)
