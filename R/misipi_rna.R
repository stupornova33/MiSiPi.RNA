...
misipi_rna <- function(vars, method = c("all", "miRNA", "piRNA", "siRNA")) {
  ...
  metadata <- data.frame(Data = character(18))
  ...
  metadata$Data[8] <- paste0("GTF File: ", vars$gtf_file)
  metadata$Data[9] <- paste0("RNAfold Path: ", vars$path_to_RNAfold)
  metadata$Data[10] <- paste0("RNAplot Path: ", vars$path_to_RNAplot)
  metadata$Data[11] <- paste0("Weighting: ", vars$weight_reads)
  metadata$Data[12] <- paste0("Annotated: ", vars$annotate_region)
  metadata$Data[13] <- paste0("Plotted: ", vars$plot_output)
  metadata$Data[14] <- paste0("Plot Type: ", vars$out_type)
  metadata$Data[15] <- paste0("Fastas Written: ", vars$write_fastas)
  metadata$Data[16] <- paste0("piRNA Palette: ", vars$pi_pal)
  metadata$Data[17] <- paste0("siRNA Palette: ", vars$si_pal)
  metadata$Data[18] <- paste0("Interactive: ", interactive())
