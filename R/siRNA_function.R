...
siRNA_function <- function(chrom_name, reg_start, reg_stop, length, min_read_count, genome_file, input_file, logfile, dir, 
  ...
  overhang_output <- overhang_output %>% dplyr::mutate(locus = chrom_name)
  overhang_output <- overhang_output[, c(10, 1:9)]
  
  suppressWarnings(
    if(!file.exists("siRNA_dicerz.txt")){
      write.table(overhang_output, file = paste0(dir, "siRNA_dicerz.txt"), sep = "\t", quote = FALSE, append = T, col.names = T, na = "NA", row.names = F)
    } else {
      write.table(overhang_output, file = paste0(dir, "siRNA_dicerz.txt"), quote = FALSE, sep = "\t", col.names = F, append = TRUE, na = "NA", row.names = F)
    }
  )
  ...
  suppressWarnings(
    if(!file.exists("siRNA_heatmap.txt")){
      write.table(heat_output, file = paste0(dir, "siRNA_heatmap.txt"), sep = "\t", quote = FALSE, append = T, col.names = F, na = "NA", row.names = F)
    } else {
      write.table(heat_output, file = paste0(dir, "siRNA_heatmap.txt"), quote = FALSE, sep = "\t", col.names = F, append = TRUE, na = "NA", row.names = F)
    }
  )
  ...
  if(!sum(results) == 0){
    ...
    suppressWarnings(
      if(!file.exists("siRNA_plus_phasedz.txt")){
        write.table(plus_phased_output, file = paste0(dir, "siRNA_plus_phasedz.txt"), sep = "\t", quote = FALSE, append = T, col.names = F, na = "NA", row.names = F)
      } else {
        write.table(plus_phased_output, file = paste0(dir, "siRNA_plus_phasedz.txt"), quote = FALSE, sep = "\t", col.names = F, append = TRUE, na = "NA", row.names = F)
      }
    )
    
    suppressWarnings(
      if(!file.exists("siRNA_minus_phasedz.txt")){
        write.table(minus_phased_output, file = paste0(dir, "siRNA_minus_phasedz.txt"), sep = "\t", quote = FALSE, append = T, col.names = F, na = "NA", row.names = F)
      } else {
        write.table(minus_phased_output, file = paste0(dir, "siRNA_minus_phasedz.txt"), quote = FALSE, sep = "\t", col.names = F, append = TRUE, na = "NA", row.names = F)
      }
    )
