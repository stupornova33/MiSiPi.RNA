...
.plot_siRNA_hp_phasing_probability_combined <- function(plus_phased_table, minus_phased_table) {
  ...
  # Add column for strand information and combine tables
  p_df <- plus_phased_table %>%
    dplyr::mutate(Strand = "+")
  m_df <- minus_phased_table %>%
    dplyr::mutate(Strand = "-")
  
  plot_df <- dplyr::bind_rows(p_df, m_df)
  
  p <- ggplot2::ggplot(plot_df, ggplot2::aes(x = phased_dist, y = phased_z, group = Strand, color = Strand)) +
...
.plot_siRNA_overhangs_combined <- function(plus_overhangs, minus_overhangs, dual_strand_overhangs) {
  ...
  m_df <- minus_overhangs %>%
    dplyr::mutate(Strand = "-")
  
  d_df <- dual_strand_overhangs %>%
    dplyr::mutate(Strand = "Dual") %>%
    dplyr::select(shift, zscore, Strand)
  
  plot_df <- dplyr::bind_rows(p_df, m_df, d_df)
  
  p <- ggplot2::ggplot(plot_df, ggplot2::aes(x = shift, y = zscore, group = Strand, color = Strand)) +
    ggplot2::geom_line(linewidth = 2) +
    ggplot2::ggtitle(plot_title) +
    ggplot2::scale_x_continuous("Shift") +
    ggplot2::scale_y_continuous("Z-score") +
    ggplot2::scale_color_manual(values = c("+" = "red", "-" = "blue", "Dual" = "black")) +
    ggplot2::theme_classic() +
    ggplot2::theme(text = ggplot2::element_text(size = 15), plot.title = ggplot2::element_text(hjust = 0.5, size = 14)) +
    ggplot2::theme(plot.margin = ggplot2::unit(c(2, 0, 0, 0), "cm"))
