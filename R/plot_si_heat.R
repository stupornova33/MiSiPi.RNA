...
plot_si_heat <- function(results, chrom_name, reg_start, reg_stop, dir, pal = c('RdYlBl', 'yelOrRed', 'MagYel', 'Greens', 'BlYel')){
   if (!missing(pal)) {
      ...
      p <- pheatmap::pheatmap(results, cluster_cols = FALSE, cluster_rows = FALSE, fontsize = 12, color = m_pal)

   } else {
      p <- pheatmap::pheatmap(results, cluster_cols = FALSE, cluster_rows = FALSE, fontsize = 12, color = grDevices::colorRampPalette('RdYlBl')(length('RdYlBl')))
   }
   return(p)
}
